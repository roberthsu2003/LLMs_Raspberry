...
本文件為教師在課堂上示範「使用者與群組」管理的完整教案，涵蓋概念說明、常用指令、實務範例與常見錯誤排除。請依序在終端機執行以下指令，並逐一解說每一步的目的與結果，讓學生能在實作中理解背後的機制。
...
在示範每個指令後，請讓學生自行練習，並即時檢查 `id`、`groups`、`ls -l` 的變化，確保概念已內化。祝教學順利！
