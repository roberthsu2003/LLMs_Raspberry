...
### 問題1:如果process在背景執行,關閉終端機,process是否還是繼續執行
**簡短答案**：**通常不會繼續執行。**
...
- `sleep 300` 只是**背景工作（background job）**
- **仍然隸屬於該終端機（TTY）**
- 終端機一關閉時：
    - 系統會送出 **`SIGHUP`（hang up）**
    - 預設行為：**程序收到 SIGHUP 就會結束**
...
| ---------- | ----------------- |
| 背景工作 (`&`) | 只是「不佔用終端機輸入」      |
| 終端機關閉      | 對所有子程序送出 `SIGHUP` |
| 預設行為       | 程序直接結束            |

👉 **`&` 只解決「佔不佔終端機」，沒有解決「存活問題」**
