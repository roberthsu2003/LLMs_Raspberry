...
class Action:
    ...
    async def action(
        ...
        # 安全解析使用者的輸入
        if not response:  # 處理 response 為 False (使用者點擊取消) 或 None 的情況
...
傳統 Action 只能取得先前講過的歷史對話。這個範例示範了**反向跟使用者要資料**。當呼叫 `type: "input"` 時，網頁上會彈出對話框，直到使用者點擊確認後，程式碼才會繼續往下走。

### 2. 安全解析輸入資料 (防止報錯)
```python
# 若點擊取消，response 可能為 False (bool)
if not response:  
    return {"status": "cancelled"}
...
if isinstance(response, str):
    ...
    user_input = response.get("message", "")
```
**這是實務上非常重要的一步！** 許多初學者會直接寫 `response.get("message")`，但如果在彈出視窗點擊「取消」，系統可能會回傳布林值 `False`；或者在不同的系統版本下，它可能直接回傳「純字串」而不是字典。由於 `bool` 和 `str` 都沒有 `.get()` 方法，直接呼叫會導致整個程式因為 `Error: 'bool' object has no attribute 'get'` 而崩潰。因此我們運用了型別判斷 (`isinstance`) 搭配防呆邏輯來確保程式的強健度。

### 3. OpenAI 相容的 API 請求與提示詞提詞 (Prompting)
```python
payload = {
    "model": self.valves.OLLAMA_MODEL,
    "messages": [
