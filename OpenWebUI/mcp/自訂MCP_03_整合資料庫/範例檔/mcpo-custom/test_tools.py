"""
測試 server.py 的 COVID-19 MCP 工具是否正常執行。

執行方式（擇一）：
  1. Docker 內（推薦）：docker compose up -d postgres && docker compose run --rm postgres-mcp python test_tools.py
  2. 本機測試：PostgreSQL 在樹莓派 Docker 內，需設定 DATABASE_URI=postgresql://pi:raspberry@<樹莓派IP>:5432/mydb

僅檢查欄位：python test_tools.py --schema-only
"""
...
if len(sys.argv) > 1 and sys.argv[1] == "--schema-only":
    from server import list_table_columns
    print("world 資料表欄位清單（請依此更新 server.py 的 SCHEMA）：\n")
    print(list_table_columns())
    sys.exit(0)

# 測試 1：匯入模組
print("=" * 50)
print("測試 1：匯入 server 模組")
print("=" * 50)
try:
    from server import (
        _q,
    ...
    try:
        ...
        if "查詢失敗" in str(result):
            ...
            if "does not exist" in result and name != "list_table_columns":
                print(f"\n  >>> 欄位不符，請查看上方 list_table_columns 的完整欄位清單，並更新 server.py 的 SCHEMA <<<\n")
        else:
            print(f"✓ {name}: {desc}")
...
if all_ok:
    ...
    print("  請確認：")
    print("  1. 推薦在 Docker 內測試：docker compose run --rm postgres-mcp python test_tools.py")
    print("  2. 本機測試（PostgreSQL 在樹莓派）：DATABASE_URI=postgresql://pi:raspberry@<樹莓派IP>:5432/mydb python test_tools.py")
