...
> 💡 **實際範例**：完整的範例專案請參考 [範例 C：TypeScript 基礎](../examples/C_ts-basic/README.md)
...
**📂 完整範例專案：** [examples/C_ts-basic](../examples/C_ts-basic/)
...
1. **實作題**：建立一個 TypeScript 專案
   ...
   - 編譯成 JavaScript
   - 比較編譯前後的差異
   - **推薦**：參考 [範例 C：TypeScript 基礎](../examples/C_ts-basic/README.md)

2. **實驗題**：故意寫一些型別錯誤，看看 TypeScript 如何提示
   - 在 `src/main.ts` 中嘗試錯誤的型別
   - 觀察編譯錯誤訊息

3. **研究題**：查看 `tsconfig.json` 的各種選項，了解它們的作用
   - 修改 `target`、`strict` 等設定
   - 觀察編譯結果的變化
